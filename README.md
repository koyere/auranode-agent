...
- Connects to local database engines as a client (PostgreSQL, MySQL/MariaDB, MongoDB, plus
  Redis status): read-only detection and exploration, a bounded SQL console, basic
  management (create/drop databases and users, passwords, grants), and compressed backups
  (dump/restore) — always as a database client, never as an OS administrator.
